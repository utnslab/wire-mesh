...
colors = ['#82B366', '#D79B00', '#6C8EBF', '#B85450', '#9673A6', '#D6B656']
markers = ['o', 'P', '^', 's', 'v', '^']
styles = ['-', '--', '-.', 'dotted']
...
for a in range(len(APPS)):
    ...
    if a == 0:
        ...
        ax.get_shared_y_axes().join(shared_y, ax)

    APP = APPS[a]
    if APP == 'reservation':
        RATES = [100, 400, 600, 800, 1000, 1200, 1500, 2000, 2500, 2800, 3000, 3200]
    elif APP == 'social':
        RATES = [100, 400, 600, 800, 1000, 1200, 1500, 2000, 2500, 2800, 3000]
    elif APP == 'boutique':
        RATES = [50, 80, 100, 120, 150, 200, 225, 250, 280, 300]
    ...
    for index in range(len(RATES)):
        flag = True
        for m in MESH:
            if RATES[index] not in latencies[MESH.index(m)]['99']:
                flag = False
        if flag:
            break
    rate = RATES[index]
    ...
    if APP == 'reservation':
        xs = [int(x) for x in np.linspace(0, 3000, 4)] if UID == 'p1' else [int(x) for x in np.linspace(0, 4000, 5)]
        xlim = [50, 3200] if UID == 'p1' else [50, 3200]
        ylim = [1, 1000]
    elif APP == 'social':
        xs = [int(x) for x in np.linspace(0, 3000, 4)] if UID == 'p1' else [int(x) for x in np.linspace(0, 4000, 5)]
        xlim = [50, 3200] if UID == 'p1' else [50, 3200]
        ...
        xs = [int(x) for x in np.linspace(0, 300, 7)] if UID == 'p1' else [int(x) for x in np.linspace(0, 500, 6)]
        xlim = [40, 260] if UID == 'p1' else [40, 320]
        ylim = [1, 1000]

    ax.set_xticks(xs)
    ax.set_xticklabels([str(x) for x in xs], fontsize=24)
    ax.set_xlim(xlim)
    ax.set_xlabel('Client Request Rate', fontsize=26)
    ...
    if a == 0:
        ax.set_ylabel('99p Latency (ms)', fontsize=26)
...
fig.tight_layout()
plt.minorticks_off()
plt.subplots_adjust(left=0.08, right=0.98, top=0.8, bottom=0.15)
