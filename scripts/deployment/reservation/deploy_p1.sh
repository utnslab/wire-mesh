...
if [ "$SCENARIO" == "wire" ]; then
  ...
  SERVICES="frontend profile search recommendation reservation user geo rate"
  ...
  ISTIO_SERVICES="frontend profile search"
  pushd $TESTBED/scripts/deployment/reservation
  ./partial_istio_init.sh $ISTIO_SERVICES
  popd

  # Add Cilium annotations for the rest of the services
  CILIUM_SERVICES="recommendation reservation user geo rate"
