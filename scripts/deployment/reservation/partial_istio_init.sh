...
SERVICES=("$@")
ALL_SERVICES=("consul" "jaeger" "frontend" "user" "search" "geo" "profile" "rate" "reccomend" "reserve")
...
do
  ...
  if [[ $service == "recommendation" ]]; then
    name="reccomend"
  elif [[ $service == "reservation" ]]; then
    name="reserve"
  fi

  for file in $TESTBED/DeathStarBench/hotelReservation/kubernetes/$name/*; do
    filename=$(basename "$file")

    # Use Istio for only service and deployment files, use kubectl for the rest
    if [[ $filename == $service"-service.yaml" || $filename == $service"-deployment.yaml" ]]; then
