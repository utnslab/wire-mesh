...
# Get the client node (last line of $HOSTS)
CLIENT_NODE=$(echo $HOSTS | tail -1 | awk '{print $1}')
...
for APP in "${APPS[@]}"; do
  ...
  ssh -o StrictHostKeyChecking=no $CONTROL_NODE "pushd \$HOME/scripts/deployment/$APP && ./deploy_p1p2.sh ${SCENARIO} && popd"
  ...
  if [[ "$SCENARIO" == "istio" ]]; then
    ...
    if [[ "$APP" == "boutique" ]]; then
      ...
      RATES=(100 500 1000 1500 2000 2400 2500)
    fi
  fi
